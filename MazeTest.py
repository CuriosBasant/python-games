...
solOn = False # Set True for solution
...
def DrawMap(gamemap, m, n, sol = []):
    ...
    line = '|' # last line
    for q in range(n-1):
        line += '_' + ('_' if gamemap[m-1][q][1]==0 else '|')
    print(line + '_>') # exit
...
def CreatePuzzle(gamemap, m, n):
    ...
    while number < area:
        ...
        while (x==m-1 or [x+1,y] in path) and (y==n-1 or [x,y+1] in path) and (x==0 or [x-1,y] in path) and (y==0 or [x,y-1] in path):
        ...
        if dir == 0: # down
            if x+1==m: continue # out of the map
            if [x+1,y] in path: continue # choose dir again
            else:
                gamemap[x][y][0]=0
                sol.append([x+1,y])
                path.append([x+1,y])
            ...
            if y+1==n: continue # out of the map
            if [x,y+1] in path: continue # choose dir again
            else:
                gamemap[x][y][1]=0
                sol.append([x,y+1])
                path.append([x,y+1])
            ...
            if x==0: continue # out of the map
            if [x-1,y] in path: continue # choose dir again
            else:
                gamemap[x-1][y][0]=0
                sol.append([x-1,y])
            ...
            if y==0: continue # out of the map
            if [x,y-1] in path: continue # choose dir again
            else:
                gamemap[x][y-1][1]=0
                sol.append([x,y-1])
...
while True:
    m = int(input('Height: '))
    try: n = int(input('Width: '))
    except: n = m

    if m<1 or n<1:
        print('Oops! The maze size can\'t be that strange >_<')
    else:
        map = CreateMap(m,n)
    ...
    if input('Do you want to Play More : ')=='n': break